...
const years: number[] = (function () {
  ...
  for (
    let yearItem = EARLIEST_REVIEW_YEAR;
...
export const zipToCoursesData = (stats: TCourseStats[]): Course[] => {
  ...
  const dataStatic: CourseDataStatic[] = mapPayloadToArray(
    coursesDataStatic,
    ...
    semesters.forEach((s) => {
      dataStatic.forEach(
        ({
          ...
          const matchingStatsAggregate = stats.find(
            (stat) =>
              stat.courseId === courseId && !stat?.year && !stat?.semesterId,
          );
          if (matchingStatsAggregate) {
            courses.push({
              ...
              isDeprecated,
              isFoundational,
              numReviews: (matchingStatsAggregate?.numReviews ??
                unreviewedCourse.numReviews)!,
              avgWorkload: (matchingStatsAggregate?.avgWorkload ??
                unreviewedCourse.avgWorkload)!,
              avgDifficulty: (matchingStatsAggregate?.avgDifficulty ??
                unreviewedCourse.avgDifficulty)!,
              avgOverall: (matchingStatsAggregate?.avgOverall ??
                unreviewedCourse.avgOverall)!,
          ...
          const matchingStatsYearSemester = stats.find(
            (stat) =>
          ...
          courses.push({
            ...
            numReviews: (matchingStatsYearSemester?.numReviews ??
              unreviewedCourse.numReviews)!,
            avgWorkload: (matchingStatsYearSemester?.avgWorkload ??
              unreviewedCourse.avgWorkload)!,
            avgDifficulty: (matchingStatsYearSemester?.avgDifficulty ??
              unreviewedCourse.avgDifficulty)!,
            avgOverall: (matchingStatsYearSemester?.avgOverall ??
              unreviewedCourse.avgOverall)!,
            avgStaffSupport: null,
            reviewsCountsByYearSem: {}, // NOTE: placeholder only; remove once migrated from Firebase to Supabase
          });
